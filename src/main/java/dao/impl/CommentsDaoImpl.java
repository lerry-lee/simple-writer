...
public class CommentsDaoImpl implements CommentsDao {
    ...
    public boolean save(CommentsEntity comments) {
        ...
        String sql = "INSERT INTO comments(sid,comment,cdate) VALUES(?,?,?)";
    ...
    public List<CommentsBean> query(int sid) {
        ...
        conn = BaseDao.getconn();
        String sql = "SELECT comment,cdate FROM comments WHERE sid=?";
        try {
            pst = conn.prepareStatement(sql);
            pst.setInt(1,sid);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                CommentsBean comments = new CommentsBean(rs.getString(1),rs.getString(2));
                list.add(comments);
            }
        } catch (SQLException e) {
            e.printStackTrace();
