...
public class ShareDaoImpl implements ShareDao {
    ...
    public boolean save(ShareEntity share) {
        ...
        String sql="INSERT INTO share (username,sdate,category,title,content) VALUES(?,?,?,?,?)";
        try{
            pst=conn.prepareStatement(sql);
            pst.setString(1,share.getUsername());
    ...
    public List<ShareEntity> query(int offset,int rows) {
        ...
        conn=BaseDao.getconn();
        String sql="SELECT SQL_CALC_FOUND_ROWS id,sdate,category,dianzan,content,title,totalComments FROM share LIMIT ?,?";
        ...
        try{
            ...
            while(rs.next()){
                ShareEntity shareEntity=new ShareEntity(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getString(5),rs.getString(6),rs.getInt(7));
